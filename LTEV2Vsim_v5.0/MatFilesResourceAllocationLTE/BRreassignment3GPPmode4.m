...
if ~isempty(stationManagement.transmittingIDsLTE(stationManagement.isRSU(stationManagement.transmittingIDsLTE)==0))     
    for i = 1:length(stationManagement.indexInActiveIDsOnlyLTE_OfTxLTE)
        ...
        BRtx = stationManagement.BRidOrigin(idVtx);
        for indexNeighborsOfVtx = 1:length(stationManagement.neighborsIDLTE(indexVtxLte,:))
           ...
           % RSU가 아닌 neighbor만
           if stationManagement.isRSU(idVrx)==1
               continue;
           end
...
if ~isempty(stationManagement.transmittingIDsLTE(stationManagement.isRSU(stationManagement.transmittingIDsLTE)==1))     
    for i = 1:length(stationManagement.indexInActiveIDsOnlyLTE_OfTxLTE)
        ...
        indexVtxLte = stationManagement.indexInActiveIDsOnlyLTE_OfTxLTE(i);
        % RSU가 다음에 쓸 자원
        BRtx = stationManagement.nextBRforRSU(idVtx);
        for indexNeighborsOfVtx = 1:length(stationManagement.neighborsIDLTE(indexVtxLte,:))
           idVrx = stationManagement.neighborsIDLTE(indexVtxLte,indexNeighborsOfVtx);
           if idVrx<=0
               break;
...
Nscheduled = length(scheduledVehicle);

% Calculate new resReselectionCounter for scheduledID
stationManagement.resReselectionCounterLTE(scheduledVehicle) = (simParams.minRandValueMode4-1) + randi((simParams.maxRandValueMode4-simParams.minRandValueMode4)+1,1,Nscheduled);
% RSU의 resReselectionCounter 새로 지정
stationManagement.resReselectionCounterLTE(scheduledRSU) = 1;
...
if simParams.probResKeep>0
    ...
    scheduledVehicle = scheduledVehicle(keepRand >= simParams.probResKeep);
...
for indexSensingV = 1:Nscheduled
    ...
    sensingMatrixScheduled = sum(stationManagement.sensingMatrixLTE(:,:,scheduledVehicle(indexSensingV)),1)/length(stationManagement.sensingMatrixLTE(:,1,1));
    ...
    knownUsedMatrixScheduled = stationManagement.knownUsedMatrixLTE(:,scheduledVehicle(indexSensingV))';
    ...
    stationManagement.BRid(scheduledVehicle(indexSensingV))=BR;
    stationManagement.BRidOrigin(scheduledVehicle(indexSensingV))=BR;
    Nreassign = Nreassign + 1;
    
    printDebugBRofMode4(timeManagement,scheduledVehicle(indexSensingV),BR,outParams);
end

% RSU의 자원 reselection
stationManagement.BRid(scheduledRSU)=stationManagement.nextBRforRSU(scheduledRSU);
stationManagement.BRidOrigin(scheduledRSU)=stationManagement.nextBRforRSU(scheduledRSU);
for i=1:length(scheduledRSU)
